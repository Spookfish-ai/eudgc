...
export class Cose1 {
  ...
  static async decodeCborAsync(data: Buffer): Promise<null | any> {
    return new Promise((resolve, reject) => {
      decodeFirst(data, function (error: any, obj: any) {
        if (error) {
  ...
  static async valueOf(encodedData: string): Promise<Cose1 | null> {
    return new Promise((resolve, reject) => {
      ...
      zlib.inflate(decodedData, function (error: any, buf: any) {
        ...
        decodeFirst(buf, function (error: any, obj: any) {
          if (error) {
            reject('failed cose1: ' + error);
            return;
          }
  ...
  makeDataForVerification() {
    ...
    const raw1 = encode(['Signature1', this.protected_]) as Uint8Array;
    const raw2 = encode([this.payload]) as Uint8Array;
